...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # Freeze only the RadImageNet DenseNet121 backbone
    ...
    amp = False  # force disable — RadImageNet backbone activations exceed FP16 range
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        # ---------------------------------------------------
        # Unfreeze DenseNet backbone after 15 epochs
        # ---------------------------------------------------
        # Determine if unfreezing backbone is required for resuming of training
        if start_epoch < 15:
            freeze = ['model.0.']
            for k, v in model.named_parameters():
                if any(x in k for x in freeze):
            ...
            for k, v in model.named_parameters():
                v.requires_grad = True
            # also apply the 10x LR reduction that epoch==15 would have applied,
            # since we're resuming past that point and optimizer param groups
            # were rebuilt fresh on resume at full LR
            for g in optimizer.param_groups:
                g["lr"] *= 0.1

        # Unfreeze DenseNet backbone
        if epoch == 15:
            LOGGER.info("========== Unfreezing DenseNet121 backbone ==========")
