...
# RadImageNet DenseNet121 backbone module
import torch.nn.functional as F
from torchvision.models import densenet121

class RadDenseNet121Backbone(nn.Module):
    """
    DenseNet121 backbone for YOLOv9, using RadImageNet-pretrained weights.
    ...
    def __init__(self, weights_path=None, out_channels=512):
        super().__init__()
        ...
        if weights_path is not None and weights_path != "":
            ...
            for k, v in ckpt.items():
                ...
                if k.startswith("backbone.0."):
                    k = k.replace("backbone.0.", "features.")
                    
                new_ckpt[k] = v

            missing1, unexpected1 = model.load_state_dict(new_ckpt, strict=False)
            print("RadImageNet DenseNet121 loaded")
            print("Missing:", missing1)
            print("Unexpected:", unexpected1)
        ...
        self.trans1 = f.transition1      # stride 8

        self.block2 = f.denseblock2      # P3, 512 channels
        self.trans2 = f.transition2      # stride 16

        self.block3 = f.denseblock3      # P4, 1024 channels
        self.trans3 = f.transition3      # stride 32

        self.block4 = f.denseblock4      # P5, 1024 channels
        self.norm5 = f.norm5

        self.p3_proj = nn.Conv2d(512, out_channels, 1)
        self.p4_proj = nn.Conv2d(1024, out_channels, 1)
        self.p5_proj = nn.Conv2d(1024, out_channels, 1)
...
# ImageNet DenseNet121 backbone module
from torchvision.models import DenseNet121_Weights

class ImageNetDenseNet121Backbone(nn.Module):
    """
    DenseNet121 backbone for YOLOv9, using standard torchvision ImageNet1K-pretrained weights.
    Structurally identical to RadDenseNet121Backbone (same extraction points, same projections) —
    the only difference is the pretrained weight source, to isolate that variable in the ablation.
    ...
    def __init__(self, weights_path=None, out_channels=512):
        ...
        # weights_path is accepted for signature compatibility with the YAML parser,
        # but ignored — always uses torchvision's standard ImageNet1K weights.
        model = densenet121(weights=DenseNet121_Weights.IMAGENET1K_V1)
        print("ImageNet-pretrained DenseNet121 loaded (torchvision IMAGENET1K_V1)")
